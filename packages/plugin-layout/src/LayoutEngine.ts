...
export abstract class LayoutEngine {
    ...
    async start(): Promise<void> {
        let allZones = [this.root, ...walker.descendants(this.root, ZoneNode)];
        await this.fillZones(allZones);
        allZones = [this.root, ...walker.descendants(this.root, ZoneNode)];
    ...
    async stop(): Promise<void> {
        for (const id in this.components) {
            for (const node of this.components[id]) {
                const zone = walker.ancestor(node, ZoneNode);
                if (zone) {
                    zone.hide(node);
                }
            }
    ...
    async prepend(componentId: ComponentId, zoneId: ZoneIdentifier, props?: {}): Promise<VNode[]> {
        const allZones = [this.root, ...walker.descendants(this.root, ZoneNode)];
    ...
    async remove(componentId: ComponentId, zoneId?: ZoneIdentifier): Promise<ZoneNode[]> {
        ...
        while ((component = components.pop())) {
            ...
            if (
                ...
                // Remove the instance.
                const zone = walker.ancestor(component, ZoneNode);
    ...
    async fillZones(nodes: VNode[]): Promise<VNode[]> {
        ...
        while (stack.length) {
            ...
            const zones = walker.descendants(node, ZoneNode);
            ...
            for (const componentId in this.componentDefinitions) {
                ...
                if (components) {
                    ...
                    matchingZones = matchingZones.filter(
                        zone => !walker.closest(zone, ancestor => components.includes(ancestor)),
                    );
                }
                if (matchingZones.length) {
                    stack.push(
    ...
    private _clear(component: VNode): void {
        const zones = walker.descendants(component, ZoneNode);
